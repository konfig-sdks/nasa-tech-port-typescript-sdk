...
import { readableStreamToString, NasaTechPortError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new NasaTechPortError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new NasaTechPortError(e, e.response?.data)
                }
                if (innerError instanceof NasaTechPortError) {
                    // Got "NasaTechPortError" from the above try block
                    throw innerError;
                }
